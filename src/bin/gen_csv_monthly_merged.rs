...
fn main() {
    ...
    for (year_month, group) in rows_grouped_by_month {
        ...
        let timestamp = chrono::NaiveDateTime::parse_from_str(&format!("{year_month}-01-01 00:00:00"), TIME_FORMAT).unwrap();
        ...
        let unix_end = timestamp_end.signed_duration_since(NaiveDateTime::UNIX_EPOCH).num_seconds() - 1;
        let unix_delta = unix_end - unix;

        let mut merged_times = group.iter()
            .filter(|x| x.5 == 1)
            .map(|x| {
